                ...
                {link.title === "Catalog" ? (
                  <div className="relative flex items-center justify-center gap-2 cursor-pointer group">
                    ...
                    <div
                          ...
                          className="invisible absolute left-[50%] translate-x-[-52%] top-full mt-2 translate-y-[1%]  
                                      flex flex-col rounded-md bg-richblack-5 p-4 group-hover:visible
                                       lg:w-[300px] z-10 text-black h-70"> 
                        ...
                        {Array.isArray(sublinks) && sublinks.length > 0 ? (
                            sublinks.map((tag, index) => (
                              <Link key={index} to={`${tag.name}`}>
                                <p
                                  className="hover:bg-slate-400 p-2 rounded mt-2"
                                  key={index}
                                >
                                  {tag.name}
                                </p>
        ...
        <div className="flex gap-4 items-center">
          {user && user?.acountType !== "Instructor" && (
            <Link to="/dashboard/cart" className="relative text-white">
              <FaShoppingCart />
              {setTotalItems > 0 && <span>{setTotalItems}</span>}
            </Link>
          )}
