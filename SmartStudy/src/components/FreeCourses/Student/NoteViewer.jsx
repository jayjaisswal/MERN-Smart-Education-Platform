...
const NoteViewer = ({ note, onClose, allNotes, onSelectNote }) => {
  ...
  const [selectedSubject, setSelectedSubject] = useState(note?.subject || null);
  ...
  const flatViewableNotes = useMemo(() => {
    const list = [];
    allNotes
      .filter((n) => n.subject === (note?.subject || selectedSubject))
      .forEach((mainNote) => {
        if (mainNote.chapters && mainNote.chapters.length > 0) {
          mainNote.chapters.forEach(ch => 
            ch.notes.forEach(sn => list.push({ ...sn, subject: mainNote.subject, chapterName: ch.name }))
          );
        } else {
          list.push(mainNote);
        }
    ...
    return list;
  }, [allNotes, note?.subject, selectedSubject]);
    ...
    <div className="flex flex-col h-full bg-richblack-900">
      <div className="p-4 border-b border-richblack-700 bg-richblack-800 flex justify-between items-center">
        <div className="flex flex-col gap-1">
          <span className="bg-indigo-600 text-white text-[10px] font-bold uppercase px-2 py-0.5 rounded w-fit tracking-wider">
              ...
              <div className="bg-richblack-900/40">
                {notes.map((n) => (
                  <div 
                    ...
                    className={`px-10 py-2.5 cursor-pointer text-sm flex items-center gap-3 transition-all ${
                      n._id === note?._id 
            ...
            <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
              ...
              <div className="truncate">
                ...
                <p className="text-[9px] md:text-[10px] text-richblack-400 uppercase tracking-tighter truncate">
                  {note?.subject} {note?.chapterName ? `• ${note?.chapterName}` : ""}
                </p>
              </div>
            </div>
