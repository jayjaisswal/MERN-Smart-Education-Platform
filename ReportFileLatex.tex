...
% ---------------------------------------------------------------
%  GNDEC INDEX HEADERS CONFIGURATION (CENTERED TITLES, ALIGNED LABELS)
% ---------------------------------------------------------------
\usepackage{tocloft}

% --- 1. TABLE OF CONTENTS: CENTERED TITLE + ALIGNED LABELS ---
\renewcommand{\cfttoctitlefont}{\hfill\normalfont\fontsize{16}{19}\selectfont\bfseries}
\renewcommand{\cftaftertoctitle}{%
  \hfill\mbox{} % These two hfills perfectly trap the main title in the center
  \vspace{0.6cm}
  \hrule height 0.6pt % Upper horizontal rule spanning across margins
  \vspace{0.2cm}
  \noindent{\normalfont\bfseries Contents}\hfill{\normalfont\bfseries Page No.}\\[-0.3cm]
  \hrule height 0.4pt % Lower thin separator rule
  \vspace{0.4cm}
}

% --- 2. LIST OF FIGURES: CENTERED TITLE + ALIGNED LABELS ---
...
\renewcommand{\cftafterloftitle}{%
  ...
  \hrule height 0.6pt
  \vspace{0.2cm}
  \noindent{\normalfont\bfseries Fig. No.\quad Figure Description}\hfill{\normalfont\bfseries Page No.}\\[-0.3cm]
  \hrule height 0.4pt
  \vspace{0.4cm}
}

% --- 3. LIST OF TABLES: CENTERED TITLE + ALIGNED LABELS ---
...
\renewcommand{\cftafterlottitle}{%
  ...
  \hrule height 0.6pt
  \vspace{0.2cm}
  \noindent{\normalfont\bfseries Table No.\quad Table Description}\hfill{\normalfont\bfseries Page No.}\\[-0.3cm]
  \hrule height 0.4pt
  \vspace{0.4cm}
}
% ---------------------------------------------------------------
...
{\fontsize{12}{15}\selectfont Batch: 2022--2026}\\[1.8cm] % Fixed spacing instead of elastic \vfill
...
\thispagestyle{fancy}
\begin{center}
{\fontsize{14}{17}\selectfont\bfseries ACCELOR MICROSYSTEMS}\\[1.5cm]
...
\begin{tikzpicture}[node distance=1.4cm and 2.5cm, auto]
  ...
  \node[entity]  (u1) {User};
  \node[process, right=2.2cm of u1] (p1) {P1: Auth\\Module};
  \node[dstore,  right=2.2cm of p1] (d1) {D1: Users};
  
  % Arrows Row 1
  \draw[myarrow] (u1) -- node[above, font=\scriptsize] {Credentials} (p1);
  \draw[myarrow] (p1) -- node[above, font=\scriptsize] {Store/Retrieve} (d1);

  % ==================== ROW 2: COURSE MANAGEMENT ====================
  \node[entity]  (u2) [below=1.6cm of u1] {Instructor};
  \node[process, right=2.2cm of u2] (p2) {P2: Course\\Management};
  \node[dstore,  right=2.2cm of p2] (d2) {D2: Courses};
  ...
  \node[entity]  (u3) [below=1.6cm of u2] {Student};
  \node[process, right=2.2cm of u3] (p3) {P3: Progress\\Tracking};
  \node[dstore,  right=2.2cm of p3] (d3) {D3: Progress};
  
  % Arrows Row 3
  \draw[myarrow] (u3) -- node[above, font=\scriptsize] {View Lecture} (p3);
  \draw[myarrow] (p3) -- node[above, font=\scriptsize] {Update Logs} (d3);

  % ==================== CROSS-ROW STRUCTURAL FEED ====================
  % Clean routing out of D2 (Courses), dropping down directly into P3 (Progress)
  \draw[myarrow, dashed, draw=gray!80] (d2.south) -- ++(0,-0.4cm) -| node[right, font=\scriptsize, pos=0.3] {Course Content} (p3.north);
